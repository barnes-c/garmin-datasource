...
import { Alert, Button, InlineField, Input, SecretInput, Stack } from '@grafana/ui';
  ...
  return (
    <>
      ...
      <InlineField
        ...
        interactive
        tooltip={
          'Optional writable path where the Garmin OAuth token is cached (same format as garmin_exporter\'s --token-file). Avoids a fresh login (and MFA code) after every Grafana restart. Leave empty to keep tokens in memory only.'
        }
      >
        <Input
      ...
      {mfaResult && (
        <Alert severity={mfaResult.ok ? 'success' : 'error'} title={mfaResult.text} />
      )}
    </>
  );
}
